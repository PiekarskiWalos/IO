...
void dodaj_transakcje(saldo*); //! funkcja dodajaca konkretne transakcje
void pokaz_kategorie(); //! funkcja pokazujaca kategorie
...
kategoria KATEGORIE[8]; //! tablica kategorii
...
void pokaz_salda() //! funkcja pokazujaca stan aktualnych sald
{
    cout<<endl;
...
bool menu_glowne(saldo* Saldo)
{
    ...
    switch(x) //! wybor menu glownego
    {
        ...
        case 3: //! wybor 3 - ustawianie limitow
                ...
                break;
		case 4:return true; //! wybor 4 - wyjœcie
...
void pokaz_kategorie()//!  funkcja wyswietlajaca nazwy kategorii z ich indeksami
{
    for(int i=0;i<=7;i++)
...
void dodaj_transakcje(saldo* Saldo)//! funkcja dodajaca transakcje
{
    ...
    cin>>k;
    cout<<"Wykonano operacje"<<endl; //! potwierdzenie dla uzytkownika
...
class transakcja //! klasa o nazwie transakcja, przechowujaca kwote i indeks kategorii
{
    double kwota;
    int index;
    //void dodaj_transakcjê();
   // {
...
for(int i;i<=7;i++)
	...
	for(;;) //! petla nieskonczona dopóki u¿ytkownik nie wyjdzie z menu g³ównego
	{
